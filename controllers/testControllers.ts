...
export default async function addToMovies(req: Request, res: Response) {
  try {
    const arrOfPrm = movies.map((mv) => {
      return {
        title: mv.Title,
        year: mv.Year,
        genre: mv.Year,
    ...
    console.log("arras is ", arrOfPrm);
    const data = await moviesModel.insertMany(arrOfPrm);
    console.log("data is", data);
    res.send(data);
    // Promise.all(arrOfPrm)
    //   .then((resolved) => {
